...
- Spec 未達可實作程度時，先辨識哪些未知會影響設計，再逐步收斂，不直接開始 Coding
- 需求存在未知時，僅對會影響設計的未知持續收斂；其餘可建立明確標示的工作假設
- 優先採用成熟方案。若已有現成方案能滿足需求，先提出並評估
- 存在多個方案時，先確認決策點，再比較方案；不要直接推薦方案而跳過決策本身
...
4. 確認本 session 以下項目，逐項討論，每項等使用者回覆後才問下一項，確認後才開始執行
    - 你理解的本次目標
    - 完成條件
    - 主要未知點
    - 若任一項本身仍有未解決的子決策，繼續追問該子決策，直到沒有遺留依賴
    - 每個問題附上你的建議答案，方便使用者直接採用或反駁
5. 執行任務中
    - 若遇到同一問題修超過兩輪未解決——停下來說明可能的根本原因，確認方向後再繼續
    - 回答一個問題前，優先利用現有資訊降低未知，而非立即向使用者提問
